<template>
    <div>
        <div class="cart">
            ...
            <a href="#">
                {{ get_carts.length }} Items - <strong>Rp.{{get_sub_total}} </strong>
                    ...
                    <div class="cart-item bb mt-10" v-for="(cart,index) in get_carts" :key="index">
                        ...
                        <div class="cart-content">
                            ...
                            <div class="d-flex justify-content-between flex-wrap">
                                <span>{{cart.qty}} x Rp.{{ cart.product.discount_price || cart.product.price }}</span>
                                <span>Rp.{{ cart.product.discount_price * cart.qty || cart.product.price * cart.qty }}</span>
                            </div>
                        </div>
                    </div>
                    <div class="total mt-10">
                        <span class="pull-left">Subtotal:</span>
                        <span class="pull-right">Rp.{{get_sub_total}}</span>
                    </div>
