$(function(){
  ...
  $('.insert_form').on('submit',function(e){
    ...
    $.ajax({
      ...
      success: (res)=>{
        ...
        $('.note-editable').html('');
        $('.preloader').hide();
        toaster('success','data iserted successfully');
      },
      error: (err)=>{
  ...
  $('.fm_file_importer').on('change', function(){
    ...
    $.post('/file-manager/store-file', formData,(res)=>{
      if(res){
        $(this).val('');
        get_all_image();
        toaster('success','Image Uploaded Successefully');
      }
    })
  })
  ...
  const activation_image_function = () => {
    selected_image = [];
  ...
  $('#fm_confirm_btn').on('click', function(e){
    ...
    if(selected_image.length){
      ...
      toaster('error', 'no file selected my bro');
    }
  })
