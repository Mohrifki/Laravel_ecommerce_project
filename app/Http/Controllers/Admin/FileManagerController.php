    ...
    public function store_file(Request $request)
    {
        if($request->hasFile('fm_file')){
            ...
            if($request->hasFile('fm_file')){
                ...
                // main image
                $path ='uploads/file_manager/fm_image_main_'.$temp_name.'.'.$extention;
                $image->save($path);
                ...
                $path ='uploads/file_manager/fm_image_848x438_'.$temp_name.'.'.$extention;
                $image->save($path);
                $this->image_save_to_db($path);
                ...
                $path ='uploads/file_manager/fm_image_400x400_'.$temp_name.'.'.$extention;
                $canvas->save($path);
                $this->image_save_to_db($path);
            }
            return($image);
